...
//		ResetPosition();
	...
	void ReloadDataFromRowIndex (int fromIndex)
	{
		...
		Vector4 finalClipRegion = panel.clipRange;
...
//			AddNewCellToLast();
	...
	protected virtual void FixedUpdate ()
	{
		...
		if (_lastOffSetY != panel.clipRange.y)
		{
...
//				Debug.Log ("Visible Last Row is "+_visibleLastRow);
			...
			_lastOffSetY = panel.clipRange.y;
		}
	}

	protected float ContentVisibleTop ()
	{
		return (/*mPanel.clipOffset.y + */(GetViewSize().y / 2)+ panel.clipRange.y);
	}

	protected float ContentVisibleBottom ()
	{
		return ((GetViewSize().y / -2) + panel.clipRange.y);
	}

	public Vector2 GetViewSize ()
	{
		return new Vector2(panel.clipRange.z, panel.clipRange.w);
	}
	...
	public delegate void FirstTimeCreateCellHandler(UITableViewCell cell);
	/// <summary>
	/// Convenience method for AttachCellWithReuseIndentifier and DequeueReusableCellWithIndentifier when caching cells.
	/// </summary>
	/// <returns>The or attach cell with reuse indetifier.</returns>
	/// <param name="onceCallback">Callback run only once when create cell</param>
	public UITableViewCell DequeueOrAttachCellWithReuseIndetifier (GameObject cellPrefab, string reuseIndentifier, FirstTimeCreateCellHandler onceCallback = null)
	{
		UITableViewCell cell = DequeueReusableCellWithIndentifier(reuseIndentifier);
		if (cell == null) 
		{
			cell = AttachCellWithReuseIndentifier(cellPrefab, reuseIndentifier);
